...
export default function TreatmentTable() {
  ...
  const loadBookings = () => {
    const allBookings = getBookingsByType('treatment')
    setBookings(allBookings)
    setStats(getBookingStats('treatment'))
  }
  ...
  const filteredBookings = filterBookingsByStatus('treatment', filter)
      ...
      <div className="bg-white rounded-lg shadow-soft overflow-hidden">
        {filteredBookings.length > 0 ? (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <a href={`tel:${booking.phone}`} className="text-green-600 hover:text-green-700">
                          {booking.phone}
                        </a>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-sm font-medium ${booking.providerType === 'doctor' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'}`}>
                        {booking.providerType === 'doctor' ? `Dr. ${booking.selectedProvider}` : booking.selectedProvider}
                      </span>
                    </td>
          ...
          <div className="text-center py-12">
            <p className="text-gray-600 text-lg">No treatments found</p>
          </div>
        )}
      </div>
    </div>
